...
export const Navbar = () => {
  ...
  return (
    <>
      {/* Main Navbar */}
      <header className="fixed top-0 left-0 right-0 z-[100] bg-[#F8F9FA] border-b border-gray-200 py-3 md:py-4 transition-all duration-300">
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          >
            <div className="relative w-48 h-14 sm:w-64 sm:h-20 md:w-96 md:h-32 flex-shrink-0">
              <Image
          ...
          <nav className="hidden lg:flex items-center gap-8 xl:gap-12">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={cn(
                  "text-sm xl:text-base font-bold tracking-wide uppercase transition-colors hover:text-[#002E5D] relative group",
                  pathname === link.href ? "text-[#002E5D]" : "text-gray-800"
                )}
              >
                {link.name}
                <span
                  className={cn(
                    "absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-[2px] bg-[#002E5D] transition-all duration-300 group-hover:w-full",
          ...
          <div className="flex items-center gap-4 md:gap-8">
            ...
            >
              <div
                className={cn(
                  "w-6 h-[3px] bg-black transition-all duration-300",
            ...
            {/* WhatsApp CTA (Black Pill Button) */}
            <a
              href="https://wa.me/260979189370"
              target="_blank"
              rel="noopener noreferrer"
              className="hidden sm:flex items-center gap-2 bg-black text-white px-6 py-3 rounded-full font-bold text-sm tracking-wide uppercase hover:bg-gray-800 transition-colors shadow-sm"
            >
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex-1 flex flex-col px-5 sm:px-8 pt-20 pb-12 max-w-md mx-auto w-full">
              {/* Close button */}
              <button
                className="self-end mb-10 p-3 -mr-3 text-gray-600 hover:text-black transition-colors"
                  ...
                  >
                    <Link
                      ...
                      className={cn(
                        ...
                        pathname === link.href
                          ? "text-[#002E5D]"
                          : "text-gray-900 hover:text-[#002E5D]"
                      )}
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      {link.name}
                      {pathname === link.href ? (
                        <span className="text-[#002E5D] text-xl font-bold">·</span>
                      ) : (
              ...
              <motion.div
                ...
                transition={{ delay: navLinks.length * 0.08 + 0.2, duration: 0.5 }}
